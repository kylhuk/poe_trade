...
- `curl -i -H "Authorization: Bearer $POE_API_OPERATOR_TOKEN" "http://127.0.0.1:8080/api/v1/ops/scanner/recommendations?sort=expected_profit_per_operation_chaos&limit=5&min_confidence=0.8"` to inspect scanner recommendations with the operation-aware ordering used by default (`expected_profit_per_operation_chaos`).
- `curl -i -H "Authorization: Bearer $POE_API_OPERATOR_TOKEN" -H "Origin: https://poe.lama-lan.ch" "http://127.0.0.1:8080/api/v1/ops/analytics/opportunities"` to inspect opportunities analytics distributions, decision-log counts, and `topOpportunities` from scanner recommendation data.
