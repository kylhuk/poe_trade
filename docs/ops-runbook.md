...
## Failure recovery
1. **Stale ingestion:** restart `market_harvester`, verify `poe_ingest_status` shows a new `last_ingest_at`, and ensure the checkpoint file under `POE_CHECKPOINT_DIR` advances before resuming downstream analytics.
