...
- OAuth 2.1 is mandatory for almost every endpoint; confidential clients (our preferred mode) keep credentials secret, can request `service:psapi`, and issue long-lived tokens, whereas public clients are limited to Authorization Code + PKCE, cannot request `service:*` scopes, and share rate limits. The CLI must keep `POE_OAUTH_CLIENT_ID/SECRET` secret, rotate tokens before 28-day expiry, and include the `User-Agent: OAuth {...}` header so GGG can identify the app [Doc: Developer Docs].
  ...
  - `stash_scribe` (`poe_trade/ingestion/stash_scribe.py`) always uses `OAuthClient` to request `constants.DEFAULT_POE_STASH_API_PATH`; keep the `client_credentials` grant type + `service:psapi` scope enforced in `oauth_client_factory` (it already raises if the scope is missing). Treat access/refresh tokens as secrets and never emit them outside the CLI.
...
### Public & private stash API semantics
- `/public-stash-tabs` accepts optional `realm` (pc/xbox/sony) and `id` (cursor from `next_change_id`) and always returns `next_change_id` plus `stashes`. Empty `stashes` means “up-to-date”, and a 5-minute delay is noted before new data surfaces, so the harvester should expect burstless polling and avoid tight loops that ignore the documented lag [Doc: API Reference].
  ...
  - `stash_scribe` hits the same path (via `constants.DEFAULT_POE_STASH_API_PATH`) and flushes snapshots into `poe_trade.raw_account_stash_snapshot` keyed by `next_change_id`; ensure the CLI never requests private stashes without `account:stashes` and that the same cursor logic prevents duplicate captures.
...
- Every response emits `X-Rate-Limit-Policy`, `X-Rate-Limit-Rules` (comma list like `ip,account,client`), `X-Rate-Limit-<rule>` (example `10:5:10` meaning 10 hits per 5s, penalized for 10s), `X-Rate-Limit-<rule>-State`, and `Retry-After` on 429s; invalid (4xx) bursts move the app toward automatic restrictions, so parse these headers on each request and obey `Retry-After` before repeating [Doc: Developer Docs].
  ...
  - For `stash_scribe`, keep the capture mutex and `_ensure_token` logic in place so parallel snapshots do not amplify the client rule counts; a single `StashScribe` instance should deliberately wait the interval parameter and rely on `rate_limit_max_retries`/`backoff` settings in `RateLimitPolicy` (already offered via CLI flags).
...
- Terms of Use clause 7 explicitly forbids scraping/mirroring data outside the documented APIs, clause 6/7/9 guard automation/macros, and third-party policy requires a visible “not affiliated” notice plus unique user agents [Doc: Terms + Developer Docs]. Cache data responsibly and, if a CLI is shipped publicly, include `This product isn't affiliated with or endorsed by Grinding Gear Games in any way.` somewhere obvious.
  ...
  - `stash_scribe` is inherently private (account-scoped) so enforce `account:stashes` (or the OAuth flow that provides it). Even though the repo currently uses only authenticated paths, any CLI extension must mention the manual OAuth registration step (`oauth@grindinggear.com`) and the macros clause if augmenting automation around captures.
...
- The exact numerical rate limits for `service:psapi` or `account:stashes` are not published; monitoring headers per response is the only reliable signal, but does the CLI need to surface thresholds to operators (e.g., log “client limit is 10/5s”)?
...
2. Treat each 429/`Retry-After` as authoritative; extend `market_harvester`/`stash_scribe` backoff logic (already present) and persist header metadata for auditing, so we can answer “which limit fired?”.
